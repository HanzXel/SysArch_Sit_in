...
$res = $conn->query("
    SELECT s.id_number, s.first_name, s.last_name, s.course, s.profile_picture,
           ...
           IFNULL(SUM(TIMESTAMPDIFF(MINUTE,
               CONCAT(si.sit_in_date,' ',si.sit_in_time),
               CONCAT(IFNULL(si.time_out_date,si.sit_in_date),' ',IFNULL(si.time_out,'00:00:00'))
           )),0) as total_min
    FROM students s
    LEFT JOIN sit_in si ON si.id_number = s.id_number AND si.time_out IS NOT NULL
    GROUP BY s.id
...
/* ── Leaderboard section ── */
.leaderboard-section{
    background: linear-gradient(180deg, var(--off-white) 0%, #e8edf8 100%);
    padding: 80px 40px;
}
...
.lb-badge{
    ...
    background:rgba(30,111,224,.1);border:1px solid rgba(30,111,224,.3);
    color:var(--blue);font-size:12px;font-weight:700;letter-spacing:.08em;
...
.lb-title{
    ...
    font-weight:600;color:var(--navy);line-height:1.2;margin-bottom:12px;
}
...
.lb-item{
    ...
    animation:lbFadeIn .5s ease both;
    border:1.5px solid transparent;
}
.lb-item:hover{
    transform:translateY(-3px);
    box-shadow:0 12px 40px rgba(10,22,40,.13);
    border-color:rgba(30,111,224,.15);
}
...
.lb-rank{
    font-size:24px;font-weight:800;color:var(--gray-300);
...
.lb-avatar{
    ...
    border:3px solid var(--gray-100);flex-shrink:0;
}
...
.lb-name{ font-size:16px;font-weight:700;color:var(--navy);margin-bottom:3px;
    white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
.lb-id{ font-size:12px;color:var(--gray-300);margin-bottom:4px; }
.lb-course{
    display:inline-block;background:rgba(30,111,224,.08);
...
@media(max-width:640px){
    .lb-item{padding:14px 16px;gap:14px;}
    .lb-avatar{width:44px;height:44px;}
    .lb-course{max-width:120px;}
    .leaderboard-section{padding:50px 20px;}
}
</style>
</head>
<body>
    ...
    <div class="lb-container">
        ...
        <div class="lb-grid">
            ...
            <div class="lb-item" style="animation-delay:<?php echo $idx*.07;?>s">
                <div class="lb-rank">
                    <?php if($rank<=3): ?>
                    <span class="lb-crown"><?php echo $medals[$rank-1]; ?></span>
                    <?php else: ?>
                    #<?php echo $rank; ?>
                    <?php endif; ?>
                </div>
