...
// ── PDF data — FIXED: server filters by date range before sending to browser ──
$pdf_students = $conn->query(
    "SELECT id_number, CONCAT(first_name,' ',last_name) as full_name, course, year_level, email, sessions, created_at
...
$pdf_from = '';
$pdf_to   = '';
if (!empty($_GET['pdf_from']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['pdf_from'])) {
    $pdf_from = $_GET['pdf_from'];
}
if (!empty($_GET['pdf_to']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['pdf_to'])) {
    $pdf_to = $_GET['pdf_to'];
}
...
$pdf_params = [];
$pdf_types  = '';
if ($pdf_from) { $pdf_where .= " AND sit_in_date >= ?"; $pdf_params[] = $pdf_from; $pdf_types .= 's'; }
if ($pdf_to)   { $pdf_where .= " AND sit_in_date <= ?"; $pdf_params[] = $pdf_to;   $pdf_types .= 's'; }

$pdf_stmt = $conn->prepare(
    "SELECT id_number, student_name, purpose, lab,
     ...
     FROM sit_in $pdf_where ORDER BY sit_in_date DESC, sit_in_time DESC LIMIT 500"
);
if ($pdf_params) { $pdf_stmt->bind_param($pdf_types, ...$pdf_params); }
$pdf_stmt->execute();
$pdf_logs = $pdf_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$pdf_stmt->close();
...
.export-date-form{margin-bottom:12px;}
.export-date-form label{font-size:12px;font-weight:600;color:var(--gray-700);margin-right:4px;}
.export-date-form input{padding:7px 10px;border:1.5px solid var(--gray-100);border-radius:var(--radius-sm);font-size:13px;font-family:'Outfit',sans-serif;color:var(--navy);background:#fafbff;outline:none;transition:all var(--transition);margin-right:8px;}
.export-date-form input:focus{border-color:var(--blue);}
.btn-pdf{display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:linear-gradient(135deg,#ef4444,#dc2626);color:var(--white);border:none;border-radius:var(--radius-sm);font-size:13px;font-weight:700;font-family:'Outfit',sans-serif;cursor:pointer;transition:all var(--transition);}
...
<div class="page-container">
    <div class="page-header">
        <div><div class="page-title">Reports &amp; Analytics</div><div class="page-subtitle">Analytics summary and PDF report generation</div></div>
    ...
    <!-- PDF Export — FIXED: date filter is a server-side GET form, not client-side JS -->
        ...
        <div class="card-body">
            <div class="export-grid">
                ...
                <div class="export-card">
                    ...
                    <div class="export-card-desc">Complete list of all registered students including ID, name, course, year, email and remaining sessions.</div>
                    <div class="export-card-actions">
                        <button class="btn-pdf" onclick="generateStudentPDF()">📥 Download PDF</button>
                ...
                <div class="export-card">
                    ...
                    <div class="export-card-desc">
                        Export session records filtered by date range. The filter is applied server-side so only matching records are included in the PDF.
                    </div>
                    <!-- FIXED: server-side date filter via GET form -->
                    <form method="GET" action="reports.php" class="export-date-form">
                        <label>From:</label><input type="date" name="pdf_from" value="<?php echo htmlspecialchars($pdf_from); ?>">
                        <label>To:</label><input type="date" name="pdf_to" value="<?php echo htmlspecialchars($pdf_to); ?>">
                        <button type="submit" class="btn btn-gray btn-sm" style="margin-top:6px;">Apply Filter</button>
                    </form>
                    <div class="export-card-actions">
                        <button class="btn-pdf" onclick="generateLogsPDF()">📥 Download PDF</button>
                        <span style="font-size:12px;color:var(--gray-300);">
                            <?php echo count($pdf_logs); ?> record<?php echo count($pdf_logs) != 1 ? 's' : ''; ?>
                            <?php if ($pdf_from || $pdf_to): ?>(filtered)<?php endif; ?>
                        </span>
                    </div>
                </div>
    ...
    <div class="charts-grid">
        <div class="card"><div class="card-header"><div class="card-header-left">Monthly Sit-ins (<?php echo date('Y'); ?>)</div></div><div class="card-body"><div class="chart-box"><canvas id="monthlyChart"></canvas></div></div></div>
        <div class="card"><div class="card-header"><div class="card-header-left">Daily Sit-ins (Last 14 Days)</div></div><div class="card-body"><div class="chart-box"><canvas id="dailyChart"></canvas></div></div></div>
        <div class="card"><div class="card-header"><div class="card-header-left">Sit-ins Per Laboratory</div></div><div class="card-body"><div class="chart-box"><canvas id="labChart"></canvas></div></div></div>
        <div class="card"><div class="card-header"><div class="card-header-left">Sit-ins By Purpose</div></div><div class="card-body"><div class="chart-box"><canvas id="purposeChart"></canvas></div></div></div>
    </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div style="background:var(--gray-100);border-radius:100px;height:8px;width:120px;overflow:hidden;"><div style="background:var(--blue);height:100%;width:<?php echo round($ts['c']/$max_c*100); ?>%;border-radius:100px;"></div></div></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<script>
const blue='#1e6fe0', blueLight='#3b82f6', accent='#64b5f6';
const defaults={plugins:{legend:{display:false}},scales:{x:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{family:'Outfit',size:12},color:'#6b7a96'}},y:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{family:'Outfit',size:12},color:'#6b7a96'},beginAtZero:true}},responsive:true,maintainAspectRatio:false};
...
new Chart(document.getElementById('purposeChart'),{type:'doughnut',data:{labels:<?php echo $purpose_labels;?>,datasets:[{data:<?php echo $purpose_values;?>,backgroundColor:['#1e6fe0','#3b82f6','#64b5f6','#0ea5e9','#0284c7','#0369a1'],borderWidth:2,borderColor:'#fff'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{family:'Outfit',size:12},color:'#374357',padding:14}}}}});

function pdfHeader(doc, title, subtitle) {
    const pageW = doc.internal.pageSize.getWidth();
    doc.setFillColor(10,22,40); doc.rect(0,0,pageW,28,'F');
    ...
    doc.setFillColor(240,244,255); doc.rect(0,28,pageW,10,'F');
...
function pdfFooter(doc) {
    const n = doc.internal.getNumberOfPages(), pageW = doc.internal.pageSize.getWidth(), pageH = doc.internal.pageSize.getHeight();
    for (let i=1;i<=n;i++){doc.setPage(i);doc.setFillColor(10,22,40);doc.rect(0,pageH-10,pageW,10,'F');doc.setFontSize(8);doc.setTextColor(255,255,255);doc.text('CCS Sit-in Monitoring System — Confidential',14,pageH-3.5);doc.text(`Page ${i} of ${n}`,pageW-14,pageH-3.5,{align:'right'});}
}
function extractTable(id) {
    const tbl = document.getElementById(id);
    return {
        head: Array.from(tbl.querySelectorAll('thead th')).map(th=>th.textContent.trim()),
        body: Array.from(tbl.querySelectorAll('tbody tr')).map(tr=>Array.from(tr.querySelectorAll('td')).map(td=>td.textContent.trim()))
    };
}
function generateStudentPDF() {
    ...
    const {head,body} = extractTable('studentTableData');
    doc.autoTable({head:[head],body,startY:y,styles:{font:'helvetica',fontSize:8,cellPadding:3,lineColor:[232,237,245],lineWidth:.3},headStyles:{fillColor:[10,22,40],textColor:255,fontStyle:'bold'},alternateRowStyles:{fillColor:[247,249,255]},margin:{left:10,right:10}});
    pdfFooter(doc);
...
function generateLogsPDF() {
    const {jsPDF} = window.jspdf;
    const doc = new jsPDF({orientation:'landscape',unit:'mm',format:'a4'});
    const from = '<?php echo $pdf_from; ?>', to = '<?php echo $pdf_to; ?>';
    let subtitle = 'Sit-in Session Records';
    if (from || to) subtitle += ` | Period: ${from||'All'} to ${to||'All'}`;
    subtitle += ' | Records: <?php echo count($pdf_logs); ?>';
    const y = pdfHeader(doc,'SIT-IN LOGS REPORT', subtitle);
    const {head,body} = extractTable('logsTableData');
    doc.autoTable({head:[head],body,startY:y,styles:{font:'helvetica',fontSize:7,cellPadding:2.5,lineColor:[232,237,245],lineWidth:.3},headStyles:{fillColor:[10,22,40],textColor:255,fontStyle:'bold'},alternateRowStyles:{fillColor:[247,249,255]},margin:{left:8,right:8}});
    pdfFooter(doc);
