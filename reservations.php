...
// Add reference_id column if it doesn't exist yet (safe migration)
...
// ── AUTO-EXPIRE: past 10-min grace window ─────────────────────────────
$conn->query("
    UPDATE reservations
    SET status = 'expired'
    WHERE status = 'approved'
      AND reservation_date = CURDATE()
      AND ADDTIME(reservation_time, '00:10:00') < CURTIME()
");

// ── Notify expired reservations (use reference_id to avoid duplicates) ─
...
");
if ($expired_q) {
    $notif_ins = $conn->prepare(
        "INSERT INTO notifications (student_id, title, message, type, reference_id) VALUES (?, 'Reservation Expired', ?, 'warning', ?)"
    );
    while ($er = $expired_q->fetch_assoc()) {
        $msg = "Your reservation for Lab {$er['lab']} (Seat {$er['seat_number']}) on "
             . date('M d', strtotime($er['reservation_date'])) . " at "
             . date('h:i A', strtotime($er['reservation_time']))
             . " has expired — you did not arrive within 10 minutes. The seat is now available.";
        $sid   = $er['student_db_id'];
        $res_id = $er['id'];
        $notif_ins->bind_param("isi", $sid, $msg, $res_id);
        $notif_ins->execute();
    }
    $notif_ins->close();
}

// ── CONVERT reservation → sit-in ────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['convert_sitin'])) {
    ...
    if ($res && in_array($res['status'], ['approved', 'pending'])) {
        if ($res['sessions'] <= 0) {
            ...
            // FIXED: no date restriction — catches open sessions from previous days too
            $dup = $conn->prepare("SELECT id FROM sit_in WHERE id_number = ? AND time_out IS NULL");
            $dup->bind_param("s", $res['id_number']);
            $dup->execute();
            if ($dup->get_result()->num_rows > 0) {
                ...
                $msg = "Your reservation for Lab {$res['lab']} (Seat {$seat_num}) has been converted to an active sit-in session.";
                $sid = $res['sdb_id'];
                $notif = $conn->prepare("INSERT INTO notifications (student_id,title,message,type,reference_id) VALUES (?,'Sit-in Started',?,'success',?)");
                $notif->bind_param("isi", $sid, $msg, $res_id);
                $notif->execute();
                $notif->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (in_array($action, ['approved', 'rejected'])) {
        ...
        if ($row) {
            ...
            $notif = $conn->prepare("INSERT INTO notifications (student_id,title,message,type,reference_id) VALUES (?,?,?,'$ty',?)");
            $notif->bind_param("issi", $sid, $t, $m, $res_id);
            $notif->execute();
            $notif->close();
        }
    }
...
if (isset($_GET['delete'])) {
    // DELETE actions via GET are acceptable here since they are admin-only
    // and protected by session; adding CSRF via a small inline form would be ideal
    $conn->query("DELETE FROM reservations WHERE id=" . (int) $_GET['delete']);
...
<div class="page-container">
    ...
    <?php if (isset($_GET['converted'])): ?><div class="alert alert-success">✓ Converted to active sit-in.</div><?php endif; ?>
    ...
    <div style="background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.25);border-radius:var(--radius-sm);padding:12px 16px;font-size:13px;color:#92400e;margin-bottom:20px;">
        ⏱️ <strong>Auto-Expire Policy:</strong> Approved reservations expire automatically if the student does not arrive within <strong>10 minutes</strong> of their scheduled time.
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="empty-state"><div class="empty-icon">📅</div><div class="empty-title">No reservations found</div></div>
            <?php else: ?>
            <div class="table-wrapper">
                <table>
                    <thead>
                        <tr><th>#</th><th>ID Number</th><th>Student</th><th>Purpose</th><th>Lab</th><th>Seat</th><th>Date</th><th>Time</th><th>Status</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                    <?php
                    $badge = ['pending'=>'badge-warning','approved'=>'badge-success','rejected'=>'badge-danger','converted'=>'badge-converted','expired'=>'badge-expired'];
                    $icons = ['pending'=>'⏳','approved'=>'✓','rejected'=>'✕','converted'=>'🟢','expired'=>'⌛'];
                    ...
                    <tr>
                        ...
                        <td><?php echo $r['seat_number'] ? '<span class="badge badge-gray">Seat ' . $r['seat_number'] . '</span>' : '—'; ?></td>
                                ...
                                <?php if (in_array($r['status'], ['pending', 'approved'])): ?>
                                    ...
                                    <form method="POST" style="display:inline;">
                                        ...
                                        <button type="submit" name="convert_sitin" class="btn-convert"
                                            onclick="return confirm('Convert this reservation to an active sit-in?')">
                                            🟢 Convert to Sit-in
                                        </button>
                                    </form>
                                    <?php endif; ?>
        ...
        <div class="modal-body">
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="admin_note" id="modal_note" class="form-input" rows="3" maxlength="500" placeholder="Add a note…"></textarea>
                </div>
