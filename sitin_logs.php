...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['timeout'])) {
    csrf_verify();
    $timeout_id = (int) $_POST['timeout'];
    $now_time   = date('H:i:s');
    $now_date   = date('Y-m-d');

    // Rebuild redirect URL preserving filters from POST
    ...
    $redirect = 'sitin_logs.php?timed_out=1' . ($qs ? '&' . $qs : '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_admin_feedback'])) {
    ...
    $rating       = (int) $_POST['admin_rating'];
    $feedback_txt = mb_substr(trim($_POST['admin_feedback_text'] ?? ''), 0, 500);
    ...
    if ($sitin_row && $rating >= 1 && $rating <= 5) {
        ...
        $ins->bind_param("iissssis", $sit_in_id, $student_db_id, $id_number, $student_name, $admin_name, $lab, $rating, $feedback_txt);
        $ins->execute();
        $ins->close();
        ...
        $notif_msg    = "Your session in Lab {$lab} on {$date_str} has been reviewed by {$admin_name}. Rating: {$star_str} ({$label}).";
        if (!empty($feedback_txt)) {
            $notif_msg .= " Feedback: \"{$feedback_txt}\"";
        }

        $notif = $conn->prepare("INSERT INTO notifications (student_id, title, message, type) VALUES (?, 'Admin Session Feedback', ?, 'info')");
        $notif->bind_param("is", $student_db_id, $notif_msg);
        $notif->execute();
        $notif->close();
    }
...
$active_count = $conn->query("SELECT COUNT(*) as c FROM sit_in WHERE time_out IS NULL")->fetch_assoc()['c'];
...
.modal-star-row label:hover,.modal-star-row label:hover ~ label,.modal-star-row input:checked ~ label{color:#f59e0b;}
...
<div class="page-container">
    ...
    <?php if (isset($_GET['timed_out'])): ?><div class="alert alert-success">✓ Student has been timed out successfully.</div><?php endif; ?>
    <?php if (isset($_GET['fb_sent'])): ?><div class="alert alert-success">✓ Feedback sent — student has been notified.</div><?php endif; ?>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php if (empty($logs)): ?>
            <div class="empty-state"><div class="empty-icon">📋</div><div class="empty-title">No sit-in records found</div><div class="empty-desc">Try adjusting the filters above.</div></div>
            <?php else: ?>
            <div class="table-wrapper">
                <table>
                    <thead>
                        <tr><th>#</th><th>ID Number</th><th>Student Name</th><th>Purpose</th><th>Lab</th><th>Sessions Left</th><th>Time In</th><th>Time Out</th><th>Duration</th><th>Status</th><th class="no-print">Actions</th></tr>
                    </thead>
                    <tbody>
                    ...
                    <tr class="<?php echo $is_active ? 'row-active' : ''; ?>">
                        ...
                        <td class="no-print">
                            <div style="display:flex;flex-direction:column;gap:5px;">
                                <?php if ($is_active): ?>
                                <form method="POST" action="sitin_logs.php" style="display:inline;"
                                      onsubmit="return confirm('Time out <?php echo htmlspecialchars(addslashes($log['student_name'])); ?>?')">
                                    <?php echo csrf_token(); ?>
                                    <input type="hidden" name="timeout"   value="<?php echo $log['id']; ?>">
                                    ...
                                    <input type="hidden" name="lab"       value="<?php echo htmlspecialchars($lab_f); ?>">
                                    <input type="hidden" name="purpose"   value="<?php echo htmlspecialchars($purpose_f); ?>">
                                    <input type="hidden" name="status"    value="<?php echo htmlspecialchars($status_f); ?>">
                                    <input type="hidden" name="p"         value="<?php echo $page; ?>">
                                    <button type="submit" class="btn-timeout">🔴 Time Out</button>
                                </form>
                                <?php else: ?>
                                <?php if ($has_admin_fb): ?>
                                <span class="btn-admin-fb-done">✓ Feedback Sent</span>
                                <?php else: ?>
                                <button class="btn-admin-fb" onclick="openAdminFb(<?php echo $log['id']; ?>,'<?php echo htmlspecialchars(addslashes($log['student_name'])); ?>','<?php echo htmlspecialchars(addslashes($log['lab'])); ?>','<?php echo date('M d, Y', strtotime($log['sit_in_date'])); ?>')">📝 Give Feedback</button>
                                <?php endif; ?>
                                <?php endif; ?>
                            </div>
                        </td>
    ...
    <div class="modal-box">
        ...
        <div class="modal-body">
            <form method="POST" action="">
                ...
                <p style="font-size:14px;color:var(--gray-500);margin-bottom:6px;">Student: <strong id="fb_student_name" style="color:var(--navy)"></strong></p>
                <p style="font-size:13px;color:var(--gray-500);margin-bottom:18px;">Lab <span id="fb_lab" style="font-weight:600;color:var(--navy)"></span> &nbsp;·&nbsp; <span id="fb_date"></span></p>
                <div class="form-group">
                    <label class="form-label">Rating <span style="color:#ef4444">*</span></label>
                    <div class="modal-star-row">
                        <input type="radio" name="admin_rating" id="as5" value="5" required><label for="as5" title="Excellent">★</label>
                        <input type="radio" name="admin_rating" id="as4" value="4"><label for="as4" title="Good">★</label>
                        <input type="radio" name="admin_rating" id="as3" value="3"><label for="as3" title="Satisfactory">★</label>
                        <input type="radio" name="admin_rating" id="as2" value="2"><label for="as2" title="Poor">★</label>
                        <input type="radio" name="admin_rating" id="as1" value="1"><label for="as1" title="Very Poor">★</label>
                    </div>
                    <div class="modal-star-hint" id="adminStarHint">Click a star to rate</div>
                </div>
                <div class="form-group">
                    <label class="form-label">Feedback / Comment <span style="font-weight:400;color:var(--gray-300)">(optional)</span></label>
                    <textarea name="admin_feedback_text" class="form-input form-textarea" rows="3" maxlength="500" placeholder="e.g. Great focus today!"></textarea>
                </div>
